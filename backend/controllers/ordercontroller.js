...
const createorder = async (req, res) => {
    try {
        ...
        // Create an array of promises to fetch product prices
        ...
        // Wait for all promises to resolve and calculate the total amount
        let totalAmounts = await Promise.all(pricePromises);
        let totalamount = totalAmounts.reduce((acc, price) => acc + price, 0);

        console.log("total", totalamount);

        // Save order details here, for example, into a database
